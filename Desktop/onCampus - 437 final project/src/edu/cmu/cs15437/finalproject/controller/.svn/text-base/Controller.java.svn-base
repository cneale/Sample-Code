...
import edu.cmu.cs15437.hw4.databeans.*;
...
public class Controller extends HttpServlet {
    ...
    public void init() throws ServletException {
        ...
        Action.add(new MyFriendListAction(model));
		Action.add(new MyEventListAction(model));
        Action.add(new LoginAction(model));
        Action.add(new LogoutAction(model));
        Action.add(new MyMessageListAction(model));
        Action.add(new ComposeAction(model));
        Action.add(new RegisterAction(model));
        Action.add(new DeleteMessageAction(model));
		Action.add(new AddFriendAction(model));
		Action.add(new AddEventAction(model));
		Action.add(new FindFriendAction(model));
		Action.add(new RemoveFriendAction(model));
		Action.add(new RemoveEventAction(model));
    ...
    private String performTheAction(HttpServletRequest request) {
        ...
        if (action.equals("start")) {
			...
			return Action.perform("events.do",request);
        }
