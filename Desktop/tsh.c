...
/*Added Functions*/
void bg_handler(struct job_t *job);
struct job_t *extractjob(struct cmdline_tokens tok);
int builtin_handler(struct cmdline_tokens tok);
void fg_handler(struct job_t *job);
 ...
 * IO_redirection
 *
 * Redirects a process' STDIN and STDOUT based user input
 */
...
/*
 * extractjob - returns information about a job based on its PID or JID
 */
...
struct job_t *extractjob(struct cmdline_tokens tok)
{
  ...
  if(tok.argv[1])
	...
	{
	  id = atoi(tok.argv[1]);
	  return getjobpid(job_list,id);
	}
      else
	{
	  id = atoi(tok.argv[1]+1);
	  return getjobjid(job_list,id);
	}
    }
  return NULL;
}
...
/*
 * fg_handler - implements the builtin_command fg
 */
...
/*
 * bg_handler - implements the builtin_command bg
 */
...
{
  int num;
  if((num=open(filename,flags,mode)) < 0)
    {
